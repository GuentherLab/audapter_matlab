function p = getAudapterDefaultParamsPitch(sex,varargin)
switch sex
    case 'male'       
        p.nLPC          = 17;
        p.fn1           = 591;
...
%% updated specifically for pitch (including upper and lower bounds above)-form time_domain_shift demo
p.dScale = 1; % no scaling for defaults
p.bTimeDomainShift = 1;
p.downFact      = 3;
p.frameLen      = 192 / p.downFact;% must be a valid DMA Buffer size (64 128 256 ..)
p.nDelay = 7; %5 in prev
p.bCepsLift = 1; %maybe need to set to zero for formants
p.timeDomainPitchShiftSchedule = 1.0;
p.rmsThresh = 0.02; %
p.sr            = 48000 / p.downFact;

%% parameters that are important for formant shifting
p.bDetect       = 1;
p.bShift        = 0; %needs to be set to 1 when doing formant shifting
p.bRatioShift   = 1;
p.bBypassFmt = 0;
p.bPitchShift = 0;
p.bTrack = 1;
...
% need to set p.bTimeDomainShift to 0 when formant shifting
...
%% see above for time domain pitchshifts
